...
import designdim from '/public/icons/designdim.svg';
import development from '/public/icons/development.svg';
import developmentdim from '/public/icons/developmentdim.svg';
import innovation from '/public/icons/innovation.svg';
import innovationdim from '/public/icons/innovationdim.svg';
import Carriage from '/public/icons/Carriage.svg';
import CoursePlan from '/public/icons/CoursePlan.svg';
import CUApts from '/public/icons/CUApts.svg';
import CUReviews from '/public/icons/CUReviews.svg';
import DAC from '/public/icons/DAC.svg';
import QMI from '/public/icons/QMI.svg';
import Zing from '/public/icons/Zing.svg';
...
const Line: React.FC = () => {
  ...
  const [isPositioned, setIsPositioned] = useState(false); // State to track if element is positioned

  const positionElementAtPoint = (
    element: HTMLElement,
    svg: SVGSVGElement,
    svgX: number,
    svgY: number
    ...
    // Calculate the scaled position based on the current size of the SVG
    const x = (svgX / svg.viewBox.baseVal.width) * svgRect.width;
    const y = (svgY / svg.viewBox.baseVal.height) * svgRect.height;

    // Position the element using absolute positioning
  ...
  useEffect(() => {
    // Set initial point in SVG coordinate system
    const svgX = 240; // example X coordinate in SVG coordinate system
    const svgY = 130; // example Y coordinate in SVG coordinate system

    const svgX2 = 260; // example X coordinate in SVG coordinate system
    const svgY2 = 875; // example Y coordinate in SVG coordinate system

    const updatePosition = () => {
      if (svgRef.current && imageRef.current) {
        positionElementAtPoint(imageRef.current, svgRef.current, svgX, svgY);
      }
      if (svgRef.current && imageRef2.current) {
        positionElementAtPoint(imageRef2.current, svgRef.current, svgX2, svgY2);
      }
    };

    updatePosition(); // Initial positioning

    // Use ResizeObserver to listen for size changes on the SVG
    ...
    return () => {
      // Cleanup: remove resize observer and window resize listener
  ...
  return (
    <>
      ...
      <div className="flex flex-col bg-black">
        {/* Begin of Line 1 Parent Container */}
        <div
          className="flex flex-col items-start relative bg-center bg-cover pb-10 sm:pb-32 md:pb-44 lg:pb-96"
          style={{ backgroundImage: "url('/images/linecomponentbackground.png')" }}
        >
          ...
          <div className="flex flex-col" ref={componentRef}>
            {!isSmall ? (
              <div className="relative">
                ...
                {/* 
              <svg className='opacity-50 static max-w-full h-[800px] sm:h-[1400px] lg:h-[2800px]' viewBox="0 0 1500 2800" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M3.93038 0.292969L969.93 585.293L973.66 587.812L969.936 590.479L312.749 1089.538L312.749 1483.37L312.985 1483.36L312.985 1607.19L1424.13 1607.19H1426.98L1426.98 1710.23L1426.98 1678.73V1781.88H1324.03L861.985        1781.88V1967.07C1084.692 1971.06 1264 2152.84 1264 2376.5C1264 2602.66 1080.661 2786 854.5 2786C628.339 2786 445 2602.66 445 2376.5C445 2150.34 628.339 1967 854.5 1967C855.061 1967 855.622 1967 856.183"
                    stroke="#FEFEFE"
                    strokeWidth="7"
                />
              </svg>
              <svg className="flex absolute top-0 max-w-full h-[800px] sm:h-[1400px] lg:h-[2800px]" height="2800" viewBox="0 0 1500 2800" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    ref={line1Ref}
                    d="M3.93038 0.292969L969.93 585.293L973.66 587.812L969.936 590.479L312.749 1089.538L312.749 1483.37L312.985 1483.36L312.985 1607.19L1424.13 1607.19H1426.98L1426.98 1710.23L1426.98 1678.73V1781.88H1324.03L861.985        1781.88V1967.07C1084.692 1971.06 1264 2152.84 1264 2376.5C1264 2602.66 1080.661 2786 854.5 2786C628.339 2786 445 2602.66 445 2376.5C445 2150.34 628.339 1967 854.5 1967C855.061 1967 855.622 1967 856.183"
                    stroke="#FEFEFE"
              ...
              <div className="relative">
                ...
                {/* <svg className='flex absolute top-0 w-screen h-auto' viewBox="0 0 1296 1996" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path ref={line1Ref} d="M619.877 1.9999L962.877 531L675.377 992L675.377 1456.5M825.876 1995.5L825.877 1653.5L969.376 1653.5L969.377 1521.5L675.377 1521.5L675.377 1421" stroke="#FEFEFE" strokeWidth="4"/>
              </svg> */}
              </div>
            )}
          </div>
          ...
          <div className={`absolute w-full text-white`}>
            {/* First Row */}
            <div className={`${isPositioned ? 'opacity-100' : 'opacity-0'}`}>
              <div
                ref={imageRef}
                style={{ left: 'initial', top: 'initial' }} // Initial fixed position; JavaScript will override
                className="flex flex-row items-center col-span-2 border-2 border-green-300 space-x-5 tracking-normal lg:tracking-widest"
              >
                <Image
                  className="h-auto w-16 sm:w-20 md:w-24 lg:w-32"
                  src={activeIcon.design ? design : designdim}
                  alt="Design Icon"
                />
                <div className="flex flex-col lg:flex-row lg:space-x-5">
                  ...
                  <p className="text-xs sm:text-sm md:text-md lg:text-2xl lg:mt-2">
                    Harnessing human-centered <br></br>design to solve problems with<br></br>{' '}
              ...
              {/* {isSmall ? (
                <div
                  className="flex flex-row items-center justify-end col-span-2 row-start-3 border-2 border-green-300 text-right space-x-5 tracking-normal"
                ...
                <div className="flex flex-row col-span-2 row-start-4 border-2 border-green-300 space-x-5 tracking-widest">
                  <Image
                    className="h-auto w-16 sm:w-20 md:w-24 lg:w-32"
                    src={activeIcon.development ? development : developmentdim}
                    alt="Development Icon"
                  ...
                  <div className="flex flex-col space-y-3">
                    <div className="text-xl lg:text-4xl font-semibold tracking-widest">
                      DEVELOPMENT
                    </div>
