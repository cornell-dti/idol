...
const Slideshow: React.FC<SlideshowProps> = ({ selectedImage }) => (
  <div className="slideshow-container">
    <img
      src={redImage.src}
      alt="DTI"
      className={`slide-image default ${selectedImage === 0 ? 'active' : ''}`}
    />
    <img
      src={purpleImage.src}
      alt="Family"
      className={`slide-image family ${selectedImage === 1 ? 'active' : ''}`}
    />
    <img
      src={blueImage.src}
      alt="Collaboration"
      className={`slide-image collaboration ${selectedImage === 2 ? 'active' : ''}`}
    />
    <img
      src={orangeImage.src}
      alt="Events"
      className={`slide-image events ${selectedImage === 3 ? 'active' : ''}`}
    />
    <img
      src={yellowImage.src}
      alt="Initiatives"
      className={`slide-image initiatives ${selectedImage === 4 ? 'active' : ''}`}
    />
  </div>
);
