...
import { signIn, signInFormExists } from './signinformAPI';
...
// Sign In Forms
loginCheckedPost('/signin/exists/:id', async (req, _) =>
  signInFormExists(req.params.id)
);
loginCheckedPost('/signin/:id', async (req, user) =>
  signIn(req.params.id, user)
);
