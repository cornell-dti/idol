...
import fs from 'fs';
import COFFEE_CHAT_BINGO_BOARD, { ADVISOR_ROLES } from '../src/consts';
...
const getMembersByCategory = async () => {
  const csv = fs.readFileSync('./scripts/coffee-chat-bingo.csv').toString();
  ...
  suggestions['is an advisor'] = filteredSuggestions(members, (mem) =>
    ADVISOR_ROLES.includes(mem.role)
  );

  suggestions['a newbie'] = filteredSuggestions(
    members,
    (mem) => mem.semesterJoined === 'Spring 2025'
  );
  ...
  const filterSelfFromCategories = (mem: IdolMember) =>
    Object.fromEntries(
      Object.entries(membersByCategory).map(([key, value]) => [
        key,
        value.filter((details) => details.netid !== mem.netid)
      ])
    );
