...
type DTI48OrganizationTree<
  M extends SimplifiedMemberForTreeGeneration = IdolMember
> = {
  readonly member: M;
  ...
  switch (member.role) {
    ...
    case 'lead':
      ...
      if (BUSINESS_LEAD_NETID === member.netid) {
        return allMembers.filter(
          (otherMember) => otherMember.role === 'business'
        );
      }
      ...
      if (DEV_LEAD_NETIDS.includes(member.netid)) {
        ...
        return allMembers.filter(
          (otherMember) => otherMember.netid === DEV_LEAD_NETIDS[index - 1]
        );
      }
      if (OPS_LEAD_NETID === member.netid) {
        return allMembers.filter(
          (otherMember) =>
            ![
              ...DEV_LEAD_NETIDS.slice(0, DEV_LEAD_NETIDS.length - 1),
              OPS_LEAD_NETID
            ].includes(otherMember.netid) && otherMember.role === 'lead'
        );
      }
      return [];
    default:
...
export const treeifyIDOLMembers = <M extends SimplifiedMemberForTreeGeneration>(
  ...
  member: start,
  children: getDirectChildrenForDTI48OrganizationTree(
    start,
    allMembers
  ).map((it) => treeifyIDOLMembers(it, allMembers))
});
...
export default function computeDTI48UpgradeChain<
  M extends SimplifiedMemberForTreeGeneration
>(targetNetID: string, allMembers: readonly M[]): readonly M[] {
