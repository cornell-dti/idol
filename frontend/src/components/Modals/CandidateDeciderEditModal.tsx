...
const CandidateDeciderEditModal: React.FC<Props> = ({ uuid, setInstances }) => {
  ...
  const handleSubmit = () => {
    if(instance) {
      ...
      CandidateDeciderAPI.updateInstance(updatedInstance).then((instance) => setInstances((instances) => instances.map((currInstance) => currInstance.uuid === instance.uuid ? instance : currInstance)));
      setIsOpen(false);
    }
  };
