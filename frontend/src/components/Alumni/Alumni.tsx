    ...
    <Container>
      ...
      {alumni?.map((alum) => (
        <Segment key={alum.uuid}>
          ...
          {alum.imageUrl && (
            <img src={alum.imageUrl} alt={`${alum.firstName} ${alum.lastName}`} width="120" height="120" />
          )}
          <p><strong>Email:</strong> {alum.email}</p>
          <p><strong>Graduation Year:</strong> {alum.gradYear || 'N/A'}</p>
          {alum.dtiRoles && alum.dtiRoles.length > 0 && (
            <p><strong>DTI Roles:</strong> {alum.dtiRoles.join(', ')}</p>
          )}
          {alum.subteams && alum.subteams.length > 0 && (
            <p><strong>Subteams:</strong> {alum.subteams.join(', ')}</p>
          )}
          <p><strong>Company:</strong> {alum.company || 'N/A'}</p>
          <p><strong>Current Role:</strong> {alum.jobRole}</p>
