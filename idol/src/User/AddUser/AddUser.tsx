...
class AddUser extends React.Component<any, AddUserState> {
          ...
          <Card.Group>
            ...
            {this.state.currentSelectedMember !== undefined ?
              <Card style={{ width: 'calc(80vw - 6rem)' }}>
                <Card.Content className={styles.cardContent}>
                  <h2 className={styles.cardHeader}>
                    {this.state.currentSelectedMember.first_name
                      + " " + this.state.currentSelectedMember.last_name}
                  </h2>
                  <Form>
                    <Form.Group widths='equal'>
                      <Form.Field
                        ...
                        onChange={(event: any) => {
                          if (this.state.currentSelectedMember) {
                            this.setState({
                              currentSelectedMember: { ...this.state.currentSelectedMember, first_name: event.target.value }
                            });
                          }
                        }}
                      ...
                      <Form.Field
                        ...
                        onChange={(event: any) => {
                          if (this.state.currentSelectedMember) {
                            this.setState({
                              currentSelectedMember: { ...this.state.currentSelectedMember, last_name: event.target.value }
                            });
                          }
                        }}
                    ...
                    <Form.Group widths='equal'>
                      {this.state.isCreatingUser ?
                        <Form.Field
                          ...
                          onChange={(event: any) => {
                            if (this.state.currentSelectedMember) {
                              this.setState({
                                currentSelectedMember: { ...this.state.currentSelectedMember, email: event.target.value }
                              });
                            }
                          }}
                      ...
                      <Form.Field
                        ...
                        onChange={(event: any, data: any) => {
                          if (this.state.currentSelectedMember) {
                            this.setState({
                              currentSelectedMember: { ...this.state.currentSelectedMember, role: data.value }
                            });
                          }
                        }}
                        value={this.state.currentSelectedMember?.role ? this.state.currentSelectedMember?.role : ''}
                      />
                    </Form.Group>
                  </Form>
                </Card.Content>
