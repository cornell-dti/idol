...
export default function SectionSep({ grid = false, hasX = false, onClickX = () => { } }: SectionSepProps) {
  if (!grid) {
    ...
    <div className="w-full overflow-hidden sectionSep border-border-1 border-l-1 border-r-1">
      <div className="flex justify-center">
        {Array.from({ length: boxCount }).map((_, i) => (
          ...
          >
            {hasX && i === lastIndex && (
              <div className="absolute cursor-pointer inset-0 flex items-center justify-center" onClick={onClickX}>
                <XIcon />
              </div>
            )}
          </div>
