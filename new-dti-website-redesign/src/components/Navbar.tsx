...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="flex justify-between items-center px-4 md:px-8 py-4 max-w-[1184px] mx-4 md:mx-32 sm:mx-8 lg:mx-auto bg-background-1 z-10 fixed [width:calc(100%-32px)] sm:[width:calc(100%-64px)] md:[width:calc(100%-256px)] lg:left-1/2 lg:-translate-x-1/2 lg:transform">
              ...
              <li key={href} className="h-10 flex items-center">
                <Link
                  ...
                  className={`transition-[color] h-10 duration-[120ms] hover:text-foreground-1 flex items-center relative focusState rounded-sm focus-visible:text-foreground-1 font-medium ${
                    pathname === href
                      ? "text-foreground-1 after:content-[''] after:absolute after:bottom-[-20px] after:left-0 after:w-full after:h-[1px] after:bg-foreground-1 after:shadow-[0_-4px_8px_0_var(--foreground-1)]"
                      : 'text-foreground-3'
                  }`}
                >
                  {label}
