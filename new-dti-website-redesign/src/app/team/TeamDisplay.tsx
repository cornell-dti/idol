...
export default function TeamDisplay() {
            ...
            <div className={`grid`} style={{ gridTemplateColumns: `repeat(${totalBoxes}, 1fr)` }}>
                {Array.from({ length: totalBoxes }, (_, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={`aspect-square relative border-r border-b border-border-1 ${isLastBox ? 'border-r-0' : ''
                                }`}
    ...
    const renderMemberGrid = (
        ...
        if (!isAlumni && selectedMember && selectedMemberIndex >= 0) {
            const shouldShowDetails = (roleName === selectedRole || selectedRole === 'Full Team') &&
                clickedSection === roleName;
